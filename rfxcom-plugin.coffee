  ...
  class RFXComPlugin extends env.plugins.Plugin
    init: (app, @framework, @config) =>
      ...
      @framework.deviceManager.on('discover', (eventData) =>
        @framework.deviceManager.discoverMessage 'pimatic-rfxcom', 'Please turn RFXCoM devices on and off.'
        ...
        setTimeout(( =>
          for device in @protocolHandler.getDevices()
            ...
            if not matched
              ...
              if device?.classname == "RfxComPirSensor" || device?.classname == "RfxComContactSensor"
                config.resetTime = 5000

              @framework.deviceManager.discoveredDevice(
                'pimatic-rfxcom', "Presence of #{device?.id} - #{device?.classname}", config
              )
        ), 5000)
      )
